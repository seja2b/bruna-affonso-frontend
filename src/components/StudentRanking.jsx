...
export default function StudentRanking() {
  ...
  const rows = useMemo(() => ranking.map((entry, index) => ({
    ...
    isCurrentUser: entry.studentId === user?.studentId || entry.email === user?.email || entry.student?.user?.email === user?.email
  })), [ranking, user])
      ...
      {rows.length === 0 ? <div className="ranking-empty">O ranking ainda não possui dados.</div> : (
        <>
          <div className="ranking-podium">
            {podium.map((row) => (
              <article key={row.id} className={`podium-card podium-${row.position} ${row.isCurrentUser ? 'current-user' : ''}`}>
                <span className="podium-position">#{row.position}</span>
                <div className="podium-avatar">{row.photo ? <img src={row.photo} alt="" /> : row.name[0]}</div>
                <strong>{row.name}</strong>
                {row.isCurrentUser && <span className="badge-current">Você</span>}
                <div className="podium-points">{row.points} pts</div>
                <small>{row.weeks} semanas concluídas</small>
              </article>
            ))}
          </div>

          {rest.length > 0 && <div className="ranking-list">
            {rest.map((row) => (
              <article key={row.id} className={`ranking-row ${row.isCurrentUser ? 'current-user' : ''}`}>
                <div className="ranking-position"><strong>{row.position}</strong></div>
